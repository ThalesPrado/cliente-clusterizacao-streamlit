...
1) Recomendação: Implementar políticas de cross-selling e upselling no segmento de restaurantes para aumentar o ticket médio do cluster 0.

2) Recomendação: Aplicação de política de descontos para fomentar o aumento do volume de vendas em períodos de baixa demanda no segmento de restaurantes do cluster 0.

3) Recomendação: Zerar ofertas para clientes do cluster com maior ticket médio como no caso dos cluster 1 e 2.
""")

st.write("""3) Dado o direcionamento de maximizar o lucro da Companhia, atual e futuro quais podem ser as sugestões?"

1) Recomendação: Reduzir CAC do cluster 0 via utilização de automação para interações de suporte e campanhas de marketing reduzindo o custo operacional atrelado a esse tipo de cluster de baixo retorno financeiro.

2) Recomendação: Otimizar preços dado a definição dos clusters pela modelagem visando aumento de margem 
...
cluster_summary = filtered_df.groupby('KMeans1 Cluster').agg({
    'ID': 'count',
    'Receita Transacional (R$)': 'sum'
...
st.write(cluster_summary[['Num_Clientes', 'Receita_Total']])

# Entradas para Migração entre Clusters
st.header("Simulação de Migração entre Clusters")
migra_2_para_1 = st.slider("Percentual de clientes migrando do Cluster 2 para o Cluster 1", min_value=0, max_value=100, value=10)
migra_1_para_0 = st.slider("Percentual de clientes migrando do Cluster 1 para o Cluster 0", min_value=0, max_value=100, value=5)

# Simulação de migração: aplicando mudanças nos números de clientes entre clusters
# Sem alterar o ticket médio, somente o número de clientes é ajustado
if 2 in cluster_summary.index and 1 in cluster_summary.index:
    # Calcular clientes migrando de Cluster 2 para 1
    clientes_migrando_2_para_1 = cluster_summary.loc[2, 'Num_Clientes'] * (migra_2_para_1 / 100)
    cluster_summary.loc[2, 'Num_Clientes'] -= clientes_migrando_2_para_1
    cluster_summary.loc[1, 'Num_Clientes'] += clientes_migrando_2_para_1

if 1 in cluster_summary.index and 0 in cluster_summary.index:
    # Calcular clientes migrando de Cluster 1 para 0
    clientes_migrando_1_para_0 = cluster_summary.loc[1, 'Num_Clientes'] * (migra_1_para_0 / 100)
    cluster_summary.loc[1, 'Num_Clientes'] -= clientes_migrando_1_para_0
    cluster_summary.loc[0, 'Num_Clientes'] += clientes_migrando_1_para_0

# A receita total de cada cluster após a migração permanece baseada na receita total inicial
# Exibir tabela final com a quantidade de clientes e receita total após a migração
st.write("### Projeção de Clientes e Receita por Cluster após Migração")
st.dataframe(cluster_summary[['Num_Clientes', 'Receita_Total']])

# Receita Total Projetada
receita_total_projetada = cluster_summary['Receita_Total'].sum()
st.write(f"**Receita Total Projetada após Migração**: R$ {receita_total_projetada:,.2f}")
