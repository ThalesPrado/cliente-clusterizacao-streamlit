...
st.dataframe(filtered_df[['ID', 'Segmento', 'Volume transacional (R$)', 'Receita Transacional (R$)', 'CAC (R$)', 'KMeans_Melhor_Modelo_Cluster']])
...
st.header("Tabela de Clientes com ticket médio por segmento")

import streamlit as st
import pandas as pd

# Suponha que `df` seja o DataFrame com os dados já classificados em clusters

# Calcular o ticket médio para cada Cluster e Segmento
df['Ticket Médio (R$)'] = df['Receita Transacional (R$)'] / df['Volume transacional (R$)']
ticket_medio_df = df.groupby(['Segmento', 'KMeans1 Cluster'])['Ticket Médio (R$)'].mean().unstack()

# Exibir a tabela no Streamlit
st.header("Ticket Médio por Cluster e Segmento")
st.write("A tabela abaixo mostra o ticket médio para cada combinação de cluster e segmento.")

st.dataframe(ticket_medio_df)

st.write("""
Essa tabela permite visualizar o ticket médio por segmento e cluster, ajudando a entender quais grupos de clientes têm maior valor médio de transação e onde podem ser aplicadas estratégias de aumento de ticket médio.
""")
...
Melhor opção baseado nos modelos treinado foi o Kmeans com 3 clusters, uma vez que apresentou um silhoutte score de 0.82(Oscila entre 1 e -1, aqui quanto maior o score melhor os pontos foram agrupados dentro do seu cluster),calinski score de 10212 (Quanto maior mais os dados homogêneos dentro de um cluster indicando proximidade entre os IDs desse cluster e inter cluster mede a distância entre os centros dos clusters, isso implica que quanto maior distância mais distintos os clientes são uns dos outros) e boldin score de 0.59 (Quanto menor esse score mais separados os clientes de comportamentos diferentes estão). 
