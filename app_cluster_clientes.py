...
- Contras: Sensível a ruídos e outliers
    ...
    st.write("Intuição: Imaginemos que contamos com dois grupos de clientes. A distância mínima considera os clientes mais próximos entre esses dois grupos para medir a distância entre os clusters. Em cada etapa o algoritmo procurar unir os clusters têm a menor distância mínima entre si.")


    st.write("#### Distância Máxima (Complete Linkage)")
    st.latex(r"d_{max}(C_i, C_j) = \max_{x \in C_i, y \in C_j} \|x - y\|")

    st.write("Intuição: Esta abordagem calcula a distância média entre os dois grupos. Isso leva em conta todos os pontos nos clusters e fornece uma essa média da distância entre eles.")
...
# Ponto Central
    st.write("#### 2. Ponto Central (Core Point)")
...
# Ponto de Borda
    st.write("#### 3. Ponto de Borda (Border Point)")
...
# Ponto de Ruído
    st.write("#### 4. Ponto de Ruído (Noise Point)")
...
st.write("""2) Estando em um cenário de concorrência acirrada: qual estratégia comercial poderíamos implementar para diminuir nossa perda de clientes e quais clientes deveriamos priorizar para blindá-los de um eventual ataque da concorrência e Por quê?"
    ...
    -Recomendação: Zerar ofertas para clientes do cluster com maior ticket médio como no caso dos cluster 1 e 2 uma vez que tem baixa sensibilidade aos preços.
""")
...
st.write("""3) Dado o direcionamento de maximizar o lucro da Companhia, atual e futuro quais podem ser as sugestões?"
    ...
    -Recomendação: Otimizar preços dado a definição dos clusters pela modelagem visando aumento de margem com preços distintos para clientes com comportamento de compra distintos.
""")
