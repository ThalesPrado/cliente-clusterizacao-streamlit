...
if 'dados_sumarizados' in globals():
    ...
    st.write(""" **Metodologia por Distância**:
...
st.write("""2) Estando em um cenário de concorrência acirrada: qual estratégia comercial poderíamos implementar para diminuir nossa perda de clientes e quais clientes deveriamos priorizar para blindá-los de um eventual ataque da concorrência e Por quê?
    ...
    -   1.Recomendação : Implementar políticas de cross-selling e upselling no segmento de restaurantes para aumentar o ticket médio do cluster 0. Isso serve para campanhas de marketing e comunicação de modo geral ofertando melhor personalização para cada agrupamento.

    -   2.Recomendação: Aplicação de política de descontos para fomentar o aumento do volume de vendas em períodos de baixa demanda no segmento de restaurantes do cluster 0. Num cenário de guerra de preços o foco deve ser em manter o volume de vendas e essa política comercial pode evitar perda de clientes.

    -   3.Recomendação: Zerar ofertas para clientes do cluster com maior ticket médio como no caso dos cluster 1 e 2 uma vez que tem baixa sensibilidade aos preços. Na teoria esses clientes não demandam desse tipo de descontos para efetuar compras.
""")
...
st.write("""3) Dado o direcionamento de maximizar o lucro da Companhia, atual e futuro quais podem ser as sugestões?"
    ...
    -   2.Recomendação: Otimizar preços dado a definição dos clusters pela modelagem visando aumento de margem com preços distintos para clientes com comportamento de compra distintos.
""")
